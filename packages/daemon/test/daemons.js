 ...
 */
export function waitForExit(proc, ms) {
    if (exited(proc))
        return Promise.resolve(true);
    return new Promise((resolve) => {
        const settle = (value) => { clearTimeout(timer); proc.off("exit", onExit); resolve(value); };
...
export async function startDaemon(opts = {}) {
    ...
    proc.unref();
    const daemon = { proc, home, port, log: () => log, stop: () => stopDaemon(daemon) };
...
export async function stopAll() {
    ...
    for (const f of failed)
        console.error(`covey test teardown: a daemon would not stop — ${f.reason}`);
}
/**
