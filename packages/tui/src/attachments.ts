...
import {
  ...
  PACK_ATTACHMENT_BYTES, SHRINK_LONG_EDGE, type Attachment,
} from "@covey/protocol";
import type { FailedDrop } from "@covey/client";
...
/**
 * The tag rules — how a pending file reads as a word in the draft, and what a
 * key that lands on one does — are the page's too, so they live in
 * `@covey/client`. They are re-exported here because this file is where the
 * TUI looks for everything about an attachment.
 */
export { SHRINK_LONG_EDGE } from "@covey/protocol";

export {
  applyDrop, chipLabel, cutTag, keepTagged, makeTag, spliceTags, tagAttachments, tagSpanAt,
