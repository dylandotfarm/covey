...
function view(items: TimelineItem[], latestTurn: string | null): ThreadView {
  ...
  return { machine: "m", threadId: "t", thread, items: new Map(items.map((i) => [i.id, i])), loading: false, error: null, hasMore: false, loadingOlder: false, commands: null, dirs: new Map() };
}
