...
async function warnIfStale(dir: string) {
  try {
    ...
    // main.js, not index.js: index.js is the version guard and almost never
    // changes, so its mtime stops tracking the build.
    const built = statSync(join(dir, "main.js")).mtimeMs;
...
async function spawnDaemon() {
  ...
  const port = localPort();
  // index.js, not main.js: the daemon we start makes the version test too.
